...
# Install
Start-Process -FilePath "$env:systemdrive\dtc\packages\pre-req-checker\PreReqChecker\RunPreReqChecks.exe" -ArgumentList "/s"
Start-Process -FilePath "msiexec.exe" -ArguementList "/i $env:systemdrive\dtc\packages\ism\windows\iDRACSvcMod.msi" /quiet /norestart"
Start-Process -FilePath "msiexec.exe" -ArguementList "/i $env:systemdrive\dtc\packages\systems-management-x64\SystemsManagementx64\SysMgmtx64.msi /quiet /norestart"
